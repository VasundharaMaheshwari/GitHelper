...
const refresh_limit = limiter({
    ...
    handler: (req,res) => {
        const date = new Date(req.rateLimit.resetTime)
        req.rateLimit.resetTime = date.toLocaleTimeString()
        return res.redirect(`/error?error_details=Too_Many_Requests_Received_Please_Wait_Till_${req.rateLimit.resetTime}`)
    }
})

module.exports = { register_limit,login_limit,refresh_limit }
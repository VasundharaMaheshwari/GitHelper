...
export const mint = async (amt: number) => {
    ...
    return { sendTokensTxSignature, postMintBalance };
};

export const transfer = async (user: string, amt: number) => {
    const deployer = await loadKeypairSignerFromFile("bosnrvc58UhxDbFb3dnjZ9rpQtepfncDmwQwCZPfd7b.json");
    const mint = await loadKeypairSignerFromFile("mntNcRwhpaBPUGggG1c4os4ZSKqpZTPWMHWWQnR6WKy.json");
    ...
    return { transferTokensTxSignature, updatedBalance, destinationWalletBalance };
};
